# Variant LoF Mechanism Interpreter (No ESM SAE)
...
* UniProt annotation / mutagenesis / variant pages — fetching the FASTA sequence is fine, reading curated functional annotations is not
...
* Do not use the ESMC SAE Forge API, do not call `ESMCForgeInferenceClient`, do not use SAE features.
...
Before running, ensure `local.yaml` has the correct `thermompnn_dir`:
...
## Step 5 — Sequence context analysis (required)

Analyse the 21-aa `SEQ_CONTEXT` window centred on the mutated residue. Consider:

- The chemical properties lost: charge, aromaticity, hydrogen-bond donor/acceptor, thiol, hydroxyl
- Known functional motifs in the window: catalytic triads (H/D/E/C/S), metal-binding signatures (HxH, CxxC, HxxH), N-glycosylation sequons (N-X-S/T where X ≠ P), phosphorylation motifs (S/T in kinase recognition contexts), disulfide-forming Cys pairs
- Whether the mutated residue is likely buried (hydrophobic → structural) or solvent-exposed (charged/polar → functional)
- Conserved positions implied by the surrounding sequence pattern
...
7. **Sequence context interpretation** — chemical property change, motif disrupted, predicted functional role of the reference residue
8. **Mechanistic conclusion** — grounded explicitly in:
   ...
   - the chemical/motif analysis of the sequence context (from Step 5)
